...
public class PrkDefPlceInfoServiceImpl implements PrkDefPlceInfoService {
    ...
    @Override
    public List<Map<String, Object>> getParkingCompletionStatus(List<Map<String, Object>> parkingList) {
        if (parkingList == null || parkingList.isEmpty()) {
            return Collections.emptyList();
        }
        try {
            Map<String, Object> params = new HashMap<>();
            params.put("parkingList", parkingList);
            return prkDefPlceInfoMapper.selectParkingCompletionStatus(params);
        } catch (Exception e) {
            log.error("❌ 작성 완료 여부 조회 실패", e);
            throw new RuntimeException("작성 완료 여부 조회 실패", e);
        }
    }
}
