...
<main class="main container">
    <div class="card">
            ...
            <section class="result-panel">
                ...
                <div class="tab-panels">
                    ...
                    <div id="panelDetail" class="tab-panel detail-wrap" role="tabpanel" aria-label="상세" style="min-width:0">
                        <div class="tab-container" id="detailTabsBar" style="display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 8px"></div>
                        <div class="tab-wrap" id="detailFrames" style="border:1px solid var(--line); border-radius:12px; background:#fff; padding:8px"></div>
                    </div>
                </div>
            </section>
        </div>
        ...
        <script>
            ...
            //const detailTabsList = $('#detailTabsList');
            const detailTabsList = $('#detailTabsBar');
            const detailFrames = $('#detailFrames');
            ...
            function applyFilter() {
                const f = {
                    ...
                    st: statusSel.value.trim(),
                    ad: addrSel.value.trim()
                };
            ...
            function activateTab(which) {
                ...
                if (listActive) {
                    ...
                    requestAnimationFrame(resizeDetail);
                }*/
                tabListBtn.classList.toggle('active');
            ...
            function buildDetailUrl(rec) {
                ...
                const file = map[rec.type] || '/prk/offparking';
            ...
            /** ===== 안전장치: 페이지 내 정적 상세 블록 제거 ===== */
            function cleanupStrayDetailBlocks() {
                const wrap = document.querySelector('.wrap');
                if (!wrap) return;
                const resultPanel = wrap.querySelector('.result-panel');
                if (!resultPanel) return;
                // result-panel 바깥 잔재 제거
                let n = resultPanel.nextElementSibling;
                while (n) {
                    const next = n.nextElementSibling;
                    wrap.removeChild(n);
                    n = next;
                }
                // result-panel 내부에서 tab-panels 이외의 정적 상세 블록 제거
                Array.from(resultPanel.children).forEach(child => {
                    if (!child.classList.contains('summary') &&
                        !child.classList.contains('tabs') &&
                        !child.classList.contains('tab-panels')) {
                        child.remove();
                    }
                });
            }
            ...
            function activateDetailTab(id) {
                openTabs.forEach(({id: tid, tabEl, panelEl}) => {
                    const active = tid === id;
                    tabEl.classList.toggle('active', active);
                    tabEl.setAttribute('aria-selected', String(active));
                    panelEl.classList.toggle('on', active);
                    panelEl.hidden = !active;
                });
                requestAnimationFrame(resizeDetail);
            }

            function addOrActivateDetail(rec) {
                const id = rec.manageNo;
                const found = openTabs.find(t => t.id === id);
                if (found) {
                    ensureDetailVisible();
                    activateDetailTab(id);
                    return;
                }
                if (openTabs.length >= MAX_DETAIL_TABS) {
                    const oldest = openTabs.shift();
                    oldest.tabEl.remove();
                    oldest.panelEl.remove();
                }
                const tab = document.createElement('div');
                tab.className = 'tab-btn';
                tab.type = 'button';
                tab.role = 'tab';
                tab.textContent = rec.nm;
                tab.title = rec.manageNo;
                ...
                detailTabsList.appendChild(tab);
                const panel = document.createElement('div');
                panel.id = `tab-${id}`;
                panel.className = 'tab';
                panel.hidden = true;
                panel.innerHTML = `
                    <h2 style="margin:6px 8px">${rec.nm}</h2>
                    <iframe title="상세: ${rec.nm}" allow="geolocation"
                ...
                // JS로만 상세 URL 주입 (EL 미사용)
                ifr.src = buildDetailUrl(rec);

                ifr.addEventListener('load', resizeDetail);
                detailFrames.appendChild(panel);
                openTabs.push({id, tabEl: tab, panelEl: panel});
                ensureDetailVisible();
                activateDetailTab(id);
            }

            function openDetail(rec) {
                addOrActivateDetail(rec);
            }
            ...
            function resizeDetail() {
                ...
                const activePane = detailFrames?.querySelector('.tab.on iframe');
                if (activePane) { activePane.style.minHeight = Math.max(400, vh - top - 16) + 'px'; }
            }
